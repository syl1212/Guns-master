...
public class DemoMessageConsumer {
    ...
    //消息处理器容器
    private static final Map<String,DemoMessageProcessor> MESSAGE_PROCESSOR_CONTAINER=new ConcurrentHashMap<String,DemoMessageProcessor>();
        ...
        for(DemoMessageProcessor messageProcessor:messageProcessors){
            ...
            if(CollectionUtils.isNotEmpty(processedRoutingKeys)){
                for(String routingKey:processedRoutingKeys){
                    MESSAGE_PROCESSOR_CONTAINER.put(routingKey,messageProcessor);
                }
            }
        }
    }
    ...
    public void onMessage(Message message, Channel channel) throws Exception {
        ...
        DemoMessageProcessor messageProcessor=MESSAGE_PROCESSOR_CONTAINER.get(receivedRoutingKey);
